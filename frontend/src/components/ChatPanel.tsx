...
export function ChatPanel({
  ...
  return (
    <div className="w-[380px] border-l border-white/10 flex flex-col shrink-0 relative overflow-hidden">
      <div
        className="absolute inset-0 bg-linear-to-b from-neutral-950 via-neutral-950/95 to-neutral-950/90 pointer-events-none"
        aria-hidden
      />
          ...
          {messages.length === 0 && clips.length === 0 ? (
            <div className="px-4 pt-6 pb-4">
              <p className="text-sm text-neutral-400 mb-4">
                Ask about the game or search for plays in the video.
              </p>
                  ...
                  >
                    <div
                      className={`max-w-[88%] text-[13px] leading-relaxed ${
                        msg.role === "user"
                          ? "bg-white/10 text-neutral-100 px-3.5 py-2 rounded-2xl rounded-br-md border border-white/15"
                          : "text-neutral-400 px-3.5 py-2 rounded-2xl rounded-bl-md bg-white/5 border border-white/10"
                      }`}
                    >
                      {msg.role === "assistant" ? (
                        <ReactMarkdown
                          components={{
                            ...
                            li: ({ children }) => (
                              <li className="text-neutral-400">{children}</li>
                            ),
                            h1: ({ children }) => (
                              <h1 className="text-sm font-semibold text-neutral-200 mb-1">
                                {children}
                              </h1>
                            ),
                            h2: ({ children }) => (
                            ...
                            h3: ({ children }) => (
                              <h3 className="text-[13px] font-semibold text-neutral-300 mb-1">
                                {children}
                              </h3>
                            ),
                            code: ({ children }) => (
                              <code className="text-amber-400/90 bg-amber-500/10 px-1 py-0.5 rounded text-[12px]">
                                {children}
                              </code>
                            ),
                      ...
                      <div className="flex justify-start">
                        <div className="max-w-[88%] rounded-xl border border-amber-500/25 bg-amber-500/5 px-4 py-3 shadow-sm">
                          <p className="text-xs text-amber-200/90 mb-3">
                            I can find video clips for this. Jump to matching
                            plays in the game.
                          </p>
                  ...
                  {msg.clips && msg.clips.length > 0 && (
                    <div className="space-y-2 mt-1">
                      <div className="flex items-center gap-2 px-0.5">
                        ...
                        <span className="text-[10px] bg-amber-500/25 text-amber-400 px-1.5 py-0.5 rounded-full font-medium border border-amber-500/30">
                          {msg.clips.length}
                        </span>
                      </div>
                      <div className="space-y-1.5">
                        {msg.clips.map((clip, ci) => {
                          const isActive =
                            clips.length > 0 &&
                            clips[currentClipIndex]?.start_time ===
                              clip.start_time;
                            ...
                            <button
                              ...
                              className={`w-full text-left rounded-xl px-3 py-2.5 transition-all border cursor-pointer backdrop-blur-sm ${
                                isActive
                                  ...
                                  : "bg-amber-500/10 border-amber-500/15 hover:bg-amber-500/15 hover:border-amber-500/25"
                              }`}
                            >
          ...
          <div
            className={`flex items-center gap-2 rounded-xl px-3.5 py-2 transition-all ${
              isLoading
                ...
                : "bg-white/5 border border-white/10 focus-within:ring-1 focus-within:ring-amber-500/20 focus-within:border-amber-500/15"
            }`}
          >
            <input
              type="text"
